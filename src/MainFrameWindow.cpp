...
namespace Application
{
	...
	Panel* MainFrameWindow::initialiseButtonPanel()
	{
		...
		sizer->Add( makeButton( panel,
								...
								[this](CommandEvent &anEvent){this->ToggleKalmanFilter(anEvent);}),
					GBPosition( 2, 0),
					GBSpan( 1, 1), EXPAND);
		sizer->Add( makeButton( panel,
								"Toggle particle filter",
								[this](CommandEvent &anEvent){this->ToggleParticleFilter(anEvent);}),
					GBPosition( 2, 1),
					GBSpan( 1, 1), EXPAND);
		sizer->Add( makeButton( panel,
								"Stop robot",
	...
	void MainFrameWindow::ToggleKalmanFilter( CommandEvent& UNUSEDPARAM(anEvent))
	{
		Logger::log("Toggling KalmanFilter");
		Model::RobotPtr robot = Model::RobotWorld::getRobotWorld().getRobot("Robot");
		if (robot && !robot->isActing())
		{
			robot->set_filter_state(Model::Robot::FilterState::kalman);
			robot->startActing();
		}
	}

	void MainFrameWindow::ToggleParticleFilter( CommandEvent& UNUSEDPARAM(anEvent))
	{
		Logger::log("Toggling particlefilter");
		Model::RobotPtr robot = Model::RobotWorld::getRobotWorld().getRobot("Robot");
		if (robot && !robot->isActing())
		{
			robot->set_filter_state(Model::Robot::FilterState::particle);
			robot->startActing();
		}
	}
