...
namespace Model
{
	...
	void Robot::drive()
	{
		try
		{
			...
			Compass compass(position, 0.0, 2.0);
			Odometer odometer(position, 0.0, 1.0);
			Matrix<double, 3, 1> kalman_point;
			...
			while (position.x > 0 && position.x < 1024 && position.y > 0 && position.y < 1024 && pathPoint < path.size())
			{
				...
				case FilterState::none:
					break;
				case FilterState::kalman:
					kalman_point = kalman_filter.calculate_kalman_filter(odometer.odometer_value(position), speed, compass.compass_direction(position));
					kalman_path.push_back(PathAlgorithm::Vertex(kalman_point.at(0, 0), kalman_point.at(1, 0)));
					break;
				case FilterState::particle:
					break;
				default:
					break;
				}
	...
	void Robot::set_filter_state(FilterState a_filter_state)
	{
		this->filter_state = a_filter_state;
	}

	PathAlgorithm::Path Robot::get_kalman_path() const
	{
		return kalman_path;
	}

	PathAlgorithm::Path Robot::get_particle_path() const
	{
