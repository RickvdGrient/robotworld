...
//		}
	...
	void RobotShape::draw( wxDC& dc)
	{
		...
		PathAlgorithm::Path kalman_path = getRobot()->get_kalman_path();
		if (kalman_path.size() != 0)
		{
			dc.SetPen( wxPen( WXSTRING( "BLACK"), borderWidth, wxPENSTYLE_SOLID));
			PathAlgorithm::Vertex& prev_vertex = kalman_path.at(0);
			for (const PathAlgorithm::Vertex& vertex : kalman_path)
			{
				dc.DrawLine(prev_vertex.asPoint(), vertex.asPoint());
				prev_vertex = vertex;
			}
		}

		PathAlgorithm::Path particle_path = getRobot()->get_particle_path();
		if (particle_path.size() != 0)
		{
			dc.SetPen( wxPen( WXSTRING( "BLACK"), borderWidth, wxPENSTYLE_SOLID));
			PathAlgorithm::Vertex& prev_vertex = particle_path.at(0);
			for (const PathAlgorithm::Vertex& vertex : particle_path)
			{
				dc.DrawLine(prev_vertex.asPoint(), vertex.asPoint());
				prev_vertex = vertex;
			}
